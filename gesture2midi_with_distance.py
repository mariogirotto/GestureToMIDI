...
class MIDIMapping:
    ...
    rotation_axis: str = 'pitch'  # 'pitch', 'yaw', 'roll', or 'distance' - which axis to use for continuous control
...
class DistanceCalibration:
    """Stores calibration data for distance estimation"""
    min_distance: float  # Minimum distance value (closest to camera)
    max_distance: float  # Maximum distance value (farthest from camera)

    def normalize(self, value: float) -> float:
        """Normalize distance to 0-1 range (0 = closest, 1 = farthest)"""
        if self.max_distance <= self.min_distance:
            return 0.5
        normalized = (value - self.min_distance) / (self.max_distance - self.min_distance)
        return float(np.clip(normalized, 0.0, 1.0))
...
class HandFeatureExtractor:
    ...
    def get_distance_estimate(self, landmarks, image_shape: Tuple[int, int]) -> float:
        """
        Estimate distance to camera based on hand size in frame.
        Returns normalized value 0-1 where 0 = closest, 1 = farthest.
        Uses the wrist-to-middle_finger_tip distance as a stable metric.
        """
        points = np.array([[lm.x, lm.y, lm.z] for lm in landmarks])

        # Use wrist to middle finger tip as primary distance metric
        # This is more stable than bounding box (which varies with finger spread)
        wrist = points[0]
        middle_tip = points[12]

        # Calculate distance in normalized coordinates (0-1 range)
        # This is independent of image resolution
        hand_length_norm = np.linalg.norm(middle_tip - wrist)

        # Also consider palm width for stability
        index_mcp = points[5]
        pinky_mcp = points[17]
        palm_width_norm = np.linalg.norm(pinky_mcp - index_mcp)

        # Combine metrics: hand length is primary, palm width adds stability
        # Weight: 70% hand length, 30% palm width
        combined_metric = 0.7 * hand_length_norm + 0.3 * palm_width_norm

        # Invert so larger hand (closer) = smaller value
        # Add small epsilon to avoid division issues
        # Typical range: 0.15 (very close) to 0.02 (far)
        distance_estimate = 1.0 / (combined_metric + 0.001)

        # Normalize to roughly 0-1 range based on expected min/max
        # These values can be calibrated per user setup
        min_expected = 5.0   # Very close
        max_expected = 50.0  # Far away

        normalized = (distance_estimate - min_expected) / (max_expected - min_expected)
        return float(np.clip(normalized, 0.0, 1.0))
    ...
    def get_global_features(self, landmarks, hand_label: str = None, 
                           image_shape: Tuple[int, int] = None) -> Dict[str, float]:
        """Get non-invariant features including distance"""
        pitch, yaw, roll = self.get_rotation_angles(landmarks, hand_label)
        # New API: landmarks is a list, access by index
        lm = landmarks

        # Get distance estimate if image shape provided
        distance = 0.5  # Default neutral
        if image_shape is not None:
            distance = self.get_distance_estimate(landmarks, image_shape)
...
class GestureRecognizer:
    ...
    def __init__(self):
        ...
        self.rotation_smoothing = {
            'Left': {'pitch': 0.5, 'yaw': 0.5, 'roll': 0.5, 'distance': 0.5, 'alpha': 0.3},
            'Right': {'pitch': 0.5, 'yaw': 0.5, 'roll': 0.5, 'distance': 0.5, 'alpha': 0.3}
        }
    ...
    def get_smooth_rotation(self, hand_label: str, new_rotation: Tuple[float, float, float], new_distance: float = None) -> Tuple[float, float, float, float]:
        ...
        # Smooth distance if provided
        if new_distance is not None:
            smooth['distance'] = alpha * new_distance + (1 - alpha) * smooth['distance']

        return (smooth['pitch'], smooth['yaw'], smooth['roll'], smooth['distance'])
...
class GestureMIDIMapper:
    def __init__(self):
        ...
        self.rotation_cc = {
            'Left': {'pitch': None, 'yaw': None, 'roll': None, 'distance': None},
            'Right': {'pitch': None, 'yaw': None, 'roll': None, 'distance': None}
        }

        # Rotation calibration storage
        self.rotation_calibration = {
            'Left': {'pitch': None, 'yaw': None, 'roll': None},
            'Right': {'pitch': None, 'yaw': None, 'roll': None}
        }

        # Distance calibration storage (separate from rotation)
        self.distance_calibration = {
            'Left': None,
            'Right': None
        }

        # Discontinuity offset configuration (0-1 range)
    ...
    def load_mappings(self):
        try:
            ...
            if '_rotation_cc' in data:
                ...
                # Ensure distance key exists in loaded config
                for hand in ['Left', 'Right']:
                    if 'distance' not in self.rotation_cc[hand]:
                        self.rotation_cc[hand]['distance'] = None
            ...
            # Load distance calibration if exists
            try:
                with open("distance_calibration.json", 'r') as f:
                    cal_data = json.load(f)
                    for hand in ['Left', 'Right']:
                        if hand in cal_data and cal_data[hand]:
                            self.distance_calibration[hand] = DistanceCalibration(
                                **cal_data[hand]
                            )
            except FileNotFoundError:
                pass
    ...
    def save_mappings(self):
        ...
        # Save distance calibration
        dist_cal_data = {}
        for hand in ['Left', 'Right']:
            if self.distance_calibration[hand]:
                dist_cal_data[hand] = asdict(self.distance_calibration[hand])
            else:
                dist_cal_data[hand] = None
        with open("distance_calibration.json", 'w') as f:
            json.dump(dist_cal_data, f, indent=2)
    ...
    def process_hand(self, landmarks, hand_label: str, frame):
        ...
        # Get image shape for distance calculation
        h, w = frame.shape[:2]
        global_features = self.recognizer.feature_extractor.get_global_features(landmarks, hand_label, (h, w))
        ...
        raw_distance = global_features['distance']

        pitch, yaw, roll, distance = self.recognizer.get_smooth_rotation(hand_label, raw_angles, raw_distance)
        ...
        cv2.putText(frame, f"P:{pitch:.2f} Y:{yaw:.2f} R:{roll:.2f} D:{distance:.2f}", 
                   (10, y_pos+30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)

        return gesture, {'pitch': pitch, 'yaw': yaw, 'roll': roll, 'distance': distance}
    ...
    def handle_midi(self, gesture: str, hand_label: str, rotation: Dict[str, float]):
        ...
        if gesture_key and gesture_key in self.mappings:
            ...
            if mapping.continuous:
                ...
                if mapping.rotation_axis == 'distance':
                    cal = self.distance_calibration[hand_label]
                    if cal:
                        value = cal.normalize(rotation['distance'])
                    else:
                        # Use legacy neutral-based calculation
                        neutral = self.gesture_neutral_rotations.get(gesture_key, {'pitch': 0.5, 'yaw': 0.5, 'roll': 0.5, 'distance': 0.5})
                        current_val = rotation['distance']
                        neutral_val = neutral['distance']
        ...
        # Handle distance CC separately
        cc_config = self.rotation_cc[hand_label]['distance']
        if cc_config is not None:
            ch, cc_num = cc_config
            cal = self.distance_calibration[hand_label]
            if cal:
    ...
    def run(self):
        ...
        print("Keys: [R]ecord  [S]ave  [M]ap  [C]C Config  [A]xis Calibrate  [D]iscontinuity Offset  [E]Distance Calibrate  [Q]uit")
        ...
        print("- Distance estimation (4th axis)")
        print("")
    ...
    def calibrate_distance(self):
        """Calibrate distance estimation with close and far positions"""
        print("\n=== Distance Calibration ===")
        print("This will calibrate the distance-to-camera estimation.")
        ...
        print(f"\nCalibrating {hand} hand distance.")
        print("You will set CLOSE (hand near camera) and FAR (hand away from camera) positions.")
        print("Press SPACE to capture each position when ready.")
        print("Press ESC to cancel at any time.\n")

        calibration_values = {'close': None, 'far': None}
        stages = ['close', 'far']
        stage_idx = 0

        while stage_idx < len(stages):
            stage = stages[stage_idx]
            print(f"\nStage {stage_idx + 1}/2: Set {stage.upper()} position")
            if stage == 'close':
                print("Hold your hand CLOSE to the camera (about 15-20cm) and press SPACE.")
            else:
                print("Hold your hand FAR from the camera (arm's length) and press SPACE.")
            ...
            while not captured:
                ...
                # Display instructions
                display_frame = np.copy(frame)
                cv2.putText(display_frame, f"Distance Cal: {hand}", (10, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                cv2.putText(display_frame, f"Stage {stage_idx + 1}/2: {stage.upper()}", (10, 60), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                cv2.putText(display_frame, "SPACE to capture, ESC to cancel", (10, 90), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
                ...
                if results.hand_landmarks:
                    for idx, landmarks in enumerate(results.hand_landmarks):
                        ...
                        if current_hand == hand:
                            h, w = frame.shape[:2]
                            current_val = self.recognizer.feature_extractor.get_distance_estimate(landmarks, (h, w))
                            ...
                            # Show current value
                            cv2.putText(display_frame, f"Current distance: {current_val:.3f}", (10, 120), 
                                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        ...
        # Validate calibration
        close_val = calibration_values['close']
        far_val = calibration_values['far']

        print(f"\n--- Calibration Summary ---")
        print(f"Close (near camera): {close_val:.3f}")
        print(f"Far (from camera):   {far_val:.3f}")

        if close_val >= far_val:
            print("\nWARNING: Close value should be smaller than far value.")
            print("(Smaller values = hand appears larger = closer to camera)")
            confirm = input("Swap values and save? (y/n): ").lower()
            if confirm == 'y':
                close_val, far_val = far_val, close_val
            else:
                print("Calibration discarded.")
                return

        if abs(far_val - close_val) < 0.1:
            print("\nWARNING: Close and Far values are very close. Calibration may not work well.")
            confirm = input("Save anyway? (y/n): ").lower()
            if confirm != 'y':
                print("Calibration discarded.")
                return
        ...
        # Save calibration
        self.distance_calibration[hand] = DistanceCalibration(
            min_distance=close_val,
            max_distance=far_val
        )

        print(f"\nDistance calibration saved for {hand} hand!")
        print("Close (near) = 0.0, Far (away) = 1.0 in MIDI output")
    ...
    def configure_rotation_cc(self):
        """Setup 3-axis rotation + distance as continuous CC"""
        ...
        print(f"\nCurrent: Pitch={self.rotation_cc[hand]['pitch']}, "
              ...
              f"Distance={self.rotation_cc[hand]['distance']}")

        axis = input("Axis (pitch/yaw/roll/distance): ").lower()
        if axis not in ['pitch', 'yaw', 'roll', 'distance']:
            return
    ...
    def map_current_gesture(self):
        ...
        rotation_axis = 'pitch'
        if msg_type == 'cc':
            rotation_axis = input("Rotation axis (pitch/yaw/roll/distance, default=pitch): ").lower() or 'pitch'
